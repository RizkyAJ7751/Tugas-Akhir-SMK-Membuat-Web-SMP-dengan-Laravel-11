                                ...
                                <tr class="hover:bg-gray-50">
                                        ...
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 w-10 h-10">
                                                <div
                                                    ...
                                                    <span
                                                        class="text-sm font-medium text-white">{{ strtoupper(substr($kontak->nama, 0, 1)) }}</span>
                                                </div>
                                            </div>
                                            <div class="ml-4">
                                                ...
                                                @if ($kontak->telepon)
                                                    <div class="text-xs text-gray-400">{{ $kontak->telepon }}</div>
                                                @endif
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        ...
                                        <div class="max-w-xs text-sm text-gray-500 truncate">
                                            {{ Str::limit($kontak->pesan, 80) }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">{{ $kontak->created_at->format('d M Y') }}</div>
                                        <div class="text-xs text-gray-500">{{ $kontak->created_at->format('H:i') }}</div>
                                    </td>
                                    <td class="px-6 py-4 text-sm font-medium whitespace-nowrap">
                                        <div class="flex items-center space-x-2">
                                            <!-- Tombol Lihat -->
                                            <a href="{{ route('admin.kontak-masuk.show', $kontak->id) }}"
                                            ...
                                            <!-- Tombol Hapus -->
                                            <form action="{{ route('admin.kontak-masuk.destroy', $kontak->id) }}"
